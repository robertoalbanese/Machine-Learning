...
%         Data type: (any class and same for all arrays)
...
% 
%   If the input arrays are table or timetable arrays, they must have the
%   same variables, and, the argument dim must always be set to 1, not 2
%   because it is not possible to duplicate variables.
% 
...
if nargin > 2
    ...
    if ~isequal(classes{:})
        error([mfilename ':inputArrays:classMismatch'],...
              'Input arrays must be the same class.')
    end
end
...
if nargin > 2
    if ~isequal(sizeArray{:})
        error([mfilename ':inputArrays:sizeMismatch'],...
...
dimIndex = reshape(1:dimLength, [], numel(varargin)).';
